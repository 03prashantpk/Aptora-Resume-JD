"""Deterministic ResumeJSON -> trusted .tex renderers for T01/T02/T03.
Carried forward faithfully from the trusted templates. Layout/typography live
here; content is data-only and LaTeX-escaped. AI/users never write LaTeX."""
from __future__ import annotations
from pathlib import Path
from .models import ResumeJSON

# Directory holding the bundled Lato TTFs. The compiler copies these into the
# compile working dir, so T01 loads REAL Lato by filename (fontspec searches the
# CWD). This avoids Windows-drive Path= quirks and works offline/deterministically.
FONTS_DIR = Path(__file__).resolve().parent.parent / "fonts" / "lato"
...
def _contact(d: ResumeJSON, serif_mail: bool = False) -> str:
    c, parts = d.contact, []
    if c.phone:
